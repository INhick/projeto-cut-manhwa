...
class ConfigManager:
    ...
    def edit_hotkeys(self, root):
        ...
        for i, (key, label) in enumerate(hotkey_labels.items()):
            ttk.Label(hotkey_window, text=label).grid(row=i, column=0, padx=10, pady=5)
            entry = ttk.Button(
                hotkey_window, 
        ...
        ttk.Button(hotkey_window, text="Salvar", command=save_hotkeys).grid(
    ...
    def save_preferences(self, root):
        ...
        ttk.Label(preferences_window, text="Tema").pack(pady=(10, 5))
...
class ModernStyledImageSlicer:
    ...
    def configure_modern_style(self):
        ...
        self.style.configure('TButton', font=('Segoe UI', 10), padding=6, relief='flat')
        self.style.map('TButton', background=[('active', '#4CAF50'), ('pressed', '#45a049')])
        self.style.configure('TLabel', font=('Segoe UI', 10), background=self.root.cget('bg'))
    ...
    def create_interface(self):
        ...
        image_frame = ttk.Frame(main_frame, borderwidth=2, relief='groove')
        ...
        self.add_images_btn = ttk.Button(control_frame, text="Adicionar Imagens", command=self.add_images)
        self.add_images_btn.pack(side=tk.LEFT, padx=5)

        self.slice_images_btn = ttk.Button(control_frame, text="Cortar Imagens", command=self.slice_images)
        self.slice_images_btn.pack(side=tk.LEFT, padx=5)

        self.clear_slices_btn = ttk.Button(control_frame, text="Limpar Cortes", command=self.clear_slices)
        self.clear_slices_btn.pack(side=tk.LEFT, padx=5)
        
        self.undo_vertical_btn = ttk.Button(
            control_frame, 
            text="↩️ Desfazer Vertical", 
            command=self.undo_vertical_cut
        )
        self.undo_vertical_btn.pack(side=tk.LEFT, padx=5)
        self.undo_vertical_btn.config(state=tk.DISABLED)
        
        self.config_btn = ttk.Button(control_frame, text="⚙️ Configurações", command=self.open_config)
        self.config_btn.pack(side=tk.LEFT, padx=5)

        self.mode_button = ttk.Button(control_frame, text="Corte Horizontal", command=self.toggle_cut_mode)
        self.mode_button.pack(side=tk.LEFT, padx=5)

        self.clear_all_images_btn = ttk.Button(control_frame, text="🗑️ Limpar Todas", command=self.clear_all_images)
        self.clear_all_images_btn.pack(side=tk.LEFT, padx=5)

        self.save_images_btn = ttk.Button(control_frame, text="💾 Salvar Imagens", command=self.save_images_without_slicing)
        self.save_images_btn.pack(side=tk.LEFT, padx=5)

        nav_frame = ttk.Frame(main_frame)
        nav_frame.pack(fill=tk.X, pady=5)

        self.prev_btn = ttk.Button(nav_frame, text="◀", width=3, command=self.previous_image)
        self.prev_btn.pack(side=tk.LEFT, padx=5)

        self.image_label = ttk.Label(nav_frame, text="Nenhuma imagem carregada", anchor='center')
        self.image_label.pack(side=tk.LEFT, expand=True)

        self.next_btn = ttk.Button(nav_frame, text="▶", width=3, command=self.next_image)
...
def main():
    root = tk.Tk()
    app = ModernStyledImageSlicer(root)
    root.mainloop()
